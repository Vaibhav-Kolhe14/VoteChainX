...
const SellToken = ({ contractInstance, erc20ContractInstance }) => {
  ...
  const sellToken = async (e) => {
    ...
    try {
      ...
      const tx = await contractWithSigner.sellVKToken(tokenValueWei);
  ...
  const approveToken = async (e) => {
    ...
    try {
      ...
      const tokenValueWei = ethers.parseEther(tokenValueEth); // Convert ETH to Wei
      const tokenMarketplace = import.meta.env.VITE_TOKENMARKETPLACE_CONTRACT_ADDRESS // Marketplace contract address
